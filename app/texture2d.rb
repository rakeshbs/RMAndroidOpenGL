class Texture2D
  ...
  def generate_texture_id(gl)
    buffer = Java::Nio::IntBuffer.allocate(1)
    gl.glGenTextures(1, buffer)
    @texture_id = buffer.array()[0]
  end

  def load_texture_from_bitmap(gl, bitmap)
    generate_texture_id(gl)
  ...
  def self.enableStates(gl)
    gl.glEnable(gl.GL_TEXTURE_2D)
    gl.glEnableClientState(gl.GL_VERTEX_ARRAY)
    gl.glEnableClientState(gl.GL_TEXTURE_COORD_ARRAY)
  end

  def self.disableStates(gl)
    gl.glDisable(gl.GL_TEXTURE_2D)
    gl.glDisableClientState(gl.GL_VERTEX_ARRAY)
    gl.glDisableClientState(gl.GL_TEXTURE_COORD_ARRAY)
  end

  def draw(gl,position,scale)
    gl.glBindTexture(gl.GL_TEXTURE_2D, @texture_id)
    gl.glPushMatrix
